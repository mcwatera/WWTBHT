...
from .scripts.ppmi.mcwatera_fp import ppmitopwords
...
#currently a timeout, takes too long!   
def ppmigraphs(request):
    template = loader.get_template('pages/graphs.html')
    results = ppmitopwords()
    
    json_dict = dict()
    for result in results.keys():
        year = str(result)
        keywords = results[results]
        json_dump = YearsOnlyCharterPPMI(keywords, year)
        json_dump = json.loads(json_dump)
        json_dict[year] = json_dump
...
def YearsOnlyCharterPPMI(results, year):
    words = []
    for key in results.keys():
        word = str(key)
        words.append(word)
